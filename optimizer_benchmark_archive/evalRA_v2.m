...
% NFE 计数: 仅当外部声明并初始化 global RA_NFE 时生效, 否则零开销
global RA_NFE %#ok<GVMIS>
if ~isempty(RA_NFE), RA_NFE = RA_NFE + 1; end
...
% 的候选仍会获得更低的 internal_J, 优化器仍能正确排序.
    ...
    %     软上界 8 防止极端风场情形失控
    % ====================================================================
    ablateWindPen = isfield(cfg,'ablate_windPenalty') && cfg.ablate_windPenalty;  % A/B 开关: 关闭搜索侧逆风罚项
    if cfg.windLookahead > 0 && nPts > 2 && ~ablateWindPen
        % v9: 复用同一硬度缩放因子
