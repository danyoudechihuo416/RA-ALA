...
%   第四部分: 可行性摘要
    ...
    fprintf('  ├─ penalty_scene_entry       = %.5f  → %.3f\n', ...
        det.penalty_scene_entry,100*det.penalty_scene_entry);
    fprintf('  └─ penalty_battery           = %.5f  → %.3f\n', ...
        det.penalty_battery,          100*det.penalty_battery);

    fprintf('  %s\n', repmat('·', 1, 64));
    fprintf('          NFZ穿越硬罚(已入J)   = %.2f\n',  det.NFZ_penalty);
    fprintf('  [诊断-不入J] smooth/turn惩罚  = %.2f   (仅 evaluateRAALASearchFitness 搜索阶段)\n', ...
        det.turn_or_smoothness_penalty);
    fprintf('  [诊断-不入J] wind_lookahead   = %.2f   (仅 evaluateRAALASearchFitness 搜索阶段)\n', ...
