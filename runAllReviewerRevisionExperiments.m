function results = runAllReviewerRevisionExperiments(userOpts)
%RUNALLREVIEWERREVISIONEXPERIMENTS Run the complete reviewer revision suite.
...
%     results = runAllReviewerRevisionExperiments(opts);
    ...
    cd(projectDir);
    logFile = fullfile(projectDir,'reviewer_revision_master_run.log');
    diary(logFile);
    cleanupDiary = onCleanup(@()diary('off')); %#ok<NASGU>

    fprintf('\n============================================================\n');
    fprintf('Reviewer revision experiment suite started: %s\n', ...
    ...
    if opts.RunMain
        ...
        if ~exist('section55_same_cohort_data.mat','file')
            error('ReviewerSuite:MissingCohort', ...
    ...
    if opts.RunBudget
        ...
        results.budget = runReviewerBudgetAnalysis(opts.BudgetOptions);
    else
        fprintf('\n[4/4] Computational-budget analysis skipped.\n');
    end

    save(fullfile(projectDir,'reviewer_revision_master_results.mat'), ...
        'results','opts');
    fprintf('\nReviewer revision suite completed.\n');
    fprintf('Master results: reviewer_revision_master_results.mat\n');
    fprintf('Log: %s\n',logFile);
end
