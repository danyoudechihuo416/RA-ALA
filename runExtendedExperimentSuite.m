...
fprintf('║  internal_J= evaluateRAALASearchFitness 内部适应度               ← 仅 RA-ALA 搜索导向用     ║\n');
fprintf('╠══════════════════════════════════════════════════════════════════════════════╣\n');
...
% ★ 统一评估口径: Top-K 阶段所有候选均用 costModel.evaluatePath 重新计算
    ...
    evalFcn    = @(x) evaluateRAALASearchFitnessForExperiment(x, param2path, t_start, hasPayload, costModel, env, cfg);
    ...
    % =========================================================
    % 捕获搜索阶段内部最优适应度.
    % evaluateRAALASearchFitness 内部 J = evaluatePath.J + smoothPenalty
    ...
    % 定向二次救援 v10 (与 runRA_ALA 完全一致 — 多位置插点)
    if ~bestFeas_f
...
%% ---- evaluateRAALASearchFitness (支持消融) ----
function J = evaluateRAALASearchFitnessForExperiment(x, param2path, t_start, hasPayload, costModel, env, cfg)
% evaluateRAALASearchFitnessForExperiment — 与 evaluateRAALASearchFitness 完全一致的校准版 (支持消融开关)
...
%   smooth ≤15, infeasible=150, NFZ ≤25, obs ≤20, headwind ≤8
    ...
    % (2b) 静态高度违规 proxy (与 runRA_ALA evaluateRAALASearchFitness 完全一致)
    ...
    %     与 evaluateRAALASearchFitness.m 保持完全一致。
...
% 与 runRA_ALA.m postSmoothRepair 完全一致 (支持消融)
