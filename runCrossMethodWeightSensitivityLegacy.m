function outputs = runCrossMethodWeightSensitivityLegacy(dataFile,userOpts)
%RUNCROSSMETHODWEIGHTSENSITIVITYLEGACY Archived cross-method implementation.
% This function is retained only for provenance and is not used by the validation suite.
%
...
% re-optimized, as is ST-EA*, for every weight setting. The other baseline paths are
...
% reported composite-cost weights.
    ...
    opts = struct('OutputDir',fullfile(projectDir,'weight_sensitivity_results'), ...
        'MakeFigure',true);
    names = fieldnames(userOpts);
    for i = 1:numel(names)
        if ~isfield(opts,names{i}), error('WeightSensitivity:UnknownOption','Unknown option: %s',names{i}); end
    ...
    checkpointDesign = struct('version',2,'provenance',provenance, ...
        'sampling',sampling,'configuration',cfg,'raSeeds',raSeeds, ...
        'analysisSource',strrep(fileread([mfilename('fullpath'),'.m']),sprintf('\r\n'),sprintf('\n')));
    % Preserve historical checkpoints from older interfaces.
    checkpointFile = fullfile(outputDir, 'weight_sensitivity_checkpoint_v2.mat');
    if isfile(checkpointFile)
        ...
        if sameDesign
            ...
            error('WeightSensitivity:CheckpointMismatch', ...
                ['Checkpoint source or cohort differs. Keep it for the matching run ', ...
                'and choose a different OutputDir for this analysis.']);
        end
    end
    ...
    fprintf('Design: %d fixed High-complexity environments x %d weight settings.\n', ...
        nEnv, nConfig);
    fprintf('RA-ALA and ST-EA* are re-optimized; other baseline paths are reused across weights.\n');
    fprintf('Planning: %.3g m; final evaluation: %.3g m.\n', ...
    ...
    for ei = 1:nEnv
        ...
        cmBase = localCreateCostModel(env, weightConfigs(1),sampling.PlanningSpacingM,sampling.MinSamples);
        ...
        for ci = 1:nConfig
            cm = localCreateCostModel(env, weightConfigs(ci),sampling.PlanningSpacingM,sampling.MinSamples);
            cmFinal = localCreateCostModel(env, weightConfigs(ci),sampling.FinalSpacingM,sampling.MinSamples);
            ...
            for ai = 1:nAlg
                ...
                try
                    ...
                    elseif ai == 4
                        algorithmSeed(row) = NaN; % deterministic space-time search
                        timer = tic;
                        ...
                        path = pathST;
                        waitSchedule = infoST.waitBeforeSegmentS;
                        pathReusedAcrossWeights(row) = false;
                    else
                        if ai == 3, algorithmSeed(row) = envSeed + 53 + ai * 11; end
                        if ~strcmp(baselineStatus{ai}, 'ok')
                            error('WeightSensitivity:BaselineFailure', '%s', baselineStatus{ai});
                        end
                        path = baselinePaths{ai};
                        planningTimeS(row) = baselinePlanningTime(ai);
                        pathReusedAcrossWeights(row) = true;
                    end

                    timer = tic;
                    [~,det] = cmFinal.evaluatePath(path,tStart,hasPayload,waitSchedule);
                    ...
                    if ~det.numerically_valid
                        ...
                        J(row)=NaN; energyWh(row)=NaN; flightTimeS(row)=NaN;
                        climbCost(row)=NaN; dynamicRisk(row)=NaN; penaltyTotal(row)=NaN;
                    end
                catch ME
                    feasible(row) = false;
    ...
    reproductionPassed = all(isfinite(expectedBaseJ)) && ...
        all(isfinite(actualBaseJ)) && maxBaseDifference <= 1e-9;
    ...
    localWriteReport(reportFile, dataFile, configTable, nEnv, cfg, ...
        reproductionPassed, maxBaseDifference, raBestCount, nConfig,sampling);
    ...
    save(fullfile(outputDir, 'weight_sensitivity_results.mat'), ...
        ...
        'maxBaseDifference','raBestCount','sampling','provenance');
...
function cm = localCreateCostModel(env, weights,spacing,minSamples)
...
function localWriteReport(reportFile, dataFile, configTable, nEnv, cfg, ...
    ...
    fprintf(fid,'Planning spacing: %.3g m; final evaluation spacing: %.3g m.\n', ...
        sampling.PlanningSpacingM,sampling.FinalSpacingM);
    fprintf(fid,'EvaluationTime_s is the separate final evaluation, including any ST-EA* wait schedule.\n');
    fprintf(fid,'Comparisons use the prespecified method-specific algorithm budgets, not equal evaluator-call budgets.\n');
