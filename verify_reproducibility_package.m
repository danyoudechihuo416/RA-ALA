...
requiredFiles = {
    ...
    fullfile('spatial_resolution_output','spatial_resolution_summary.csv')
    fullfile('spatial_resolution_output','spatial_resolution_stability.csv')
    ...
    fullfile('computational_budget_output','planner_runtime_raw.csv')};
...
resolutionCaseT = readtable(fullfile(root,'spatial_resolution_output', ...
    'spatial_resolution_case_results.csv'),'TextType','string');
resolutionCaseVars = {'penalty_scene_entry','scene_entry_violation'};
assert(all(ismember(resolutionCaseVars,resolutionCaseT.Properties.VariableNames)) && ...
       ~any(ismember({'penalty_dynamic','dynamic_violation'}, ...
       resolutionCaseT.Properties.VariableNames)), ...
    'Resolution case fields are not synchronized with scene-entry terminology.');
resolutionSummaryT = readtable(fullfile(root,'spatial_resolution_output', ...
    'spatial_resolution_summary.csv'),'TextType','string');
assert(ismember('scene_entry_violation_count', ...
       resolutionSummaryT.Properties.VariableNames) && ...
       ~ismember('dynamic_violation_count', ...
       resolutionSummaryT.Properties.VariableNames), ...
    'Resolution summary fields retain deprecated dynamic-violation terminology.');
resolutionStabilityT = readtable(fullfile(root,'spatial_resolution_output', ...
    'spatial_resolution_stability.csv'),'TextType','string');
assert(ismember('scene_entry_status_agreement', ...
       resolutionStabilityT.Properties.VariableNames) && ...
       ~ismember('dynamic_status_agreement', ...
       resolutionStabilityT.Properties.VariableNames), ...
    'Resolution stability fields retain deprecated dynamic-status terminology.');

budgetRawT = readtable(fullfile(root,'computational_budget_output', ...
    'planner_runtime_raw.csv'),'TextType','string');
assert(ismember('generation_success',budgetRawT.Properties.VariableNames) && ...
       ~ismember('planner_success',budgetRawT.Properties.VariableNames), ...
    'Budget CSV must distinguish path-generation success from final feasibility.');
...
runtimeT = readtable(fullfile(root,'manuscript_completed_results', ...
    'planner_runtime_summary.csv'),'TextType','string');
for a = 1:numel(S.algNames)
    use = logical(S.stat_is_unique_trial(a,:));
    rt = S.stat_runtime_s(a,use);
    calls = S.stat_evaluator_calls(a,use);
    expected = [median(rt,'omitnan'),prctile(rt,25),prctile(rt,75), ...
        median(calls,'omitnan'),prctile(calls,25),prctile(calls,75)];
    row = runtimeT.planner==string(S.algNames{a});
    assert(sum(row)==1,'Runtime summary must contain one row per planner.');
    actual = [runtimeT.runtime_median_s(row),runtimeT.runtime_q1_s(row), ...
        runtimeT.runtime_q3_s(row),runtimeT.evaluator_calls_median(row), ...
        runtimeT.evaluator_calls_q1(row),runtimeT.evaluator_calls_q3(row)];
    assert(all(abs(actual-expected)<1e-8), ...
        'Runtime quartiles do not match MATLAB prctile for %s.',S.algNames{a});
end
